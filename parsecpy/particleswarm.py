...
"""
    ...
    based on data from Parsec applications execution.
...
class Particle:
    """
    Class that represent a particle os swarm
    ...
    def update(self, bestparticula, w, c1, c2):
        ...
        self.vel = constricao * (self.vel + c1 * rp * (self.bestpos - self.pos) + c2 * rg * (bestparticula.pos - self.pos))
...
class Swarm:
    ...
    def __init__(self, lxmin, lxmax, modelpath, size=100, w=0.5, c1=2, c2=2, maxiter=100,
        ...
        bestfpos = [p.setfpos(newfpos[i]) for i, p in
    ...
    def _swarm_med(self):
        ...
        return abs(med/self.bestparticle.fpos - 1)
    ...
    def run(self):
        ...
        while (self._swarm_med() > 1e-5 or gbestmax < 10) and iter < self.maxiter:
            ...
            if self.threads > 1:
                ...
                for i, p in enumerate(self.particles):
                    newfpos[i] = self.obj(p)
            bestfpos = [p.setfpos(newfpos[i]) for i,p in enumerate(self.particles)]
            self.bestparticle = copy.copy(self.particles[np.argmin(bestfpos)])
