...
"""
    ...
    parsecpy_createinputs [-h] -p {freqmine,fluidanimate,x264} -n NUMBEROFPARTS
        ...
        optional arguments
            ...
            -p {freqmine,fluidanimate, x264}, --package {freqmine,fluidanimate, x264}
...
# Template with 1 argument: filename.runconf
...
def dedup_splitdiff(tfile, n):
    ...
    for ftn,fn in lfile:
        ...
        fpath2 = os.path.join(pkgconffolder,fnbase+'.runconf')
        fconf2 = open(fpath2,'w')
        fconf2.write(template_freqmine_inputconf % (fnbase+'.runconf'))
        fconf2.close()
    print('Sucessful finished split operation. Total parts = ',partscount)
...
def argsparsevalidation():
    ...
    packagechoice = ['freqmine', 'fluidanimate', 'x264']
    ...
    if os.path.isfile(args.inputfilename):
        if args.package == 'freqmine':
            ...
            if args.typeofsplit == 'diff':
                ...
                print("Error: 'diff' type of part split only.")
    else:
