...
"""
  ...
  Its possible run on loop to repeat the same sigle parsec execution on
  specific number of times; And, also, its possiblme refer to multiples
  input sizes on the same execution.
...
def main():
    ...
    Trigger a sequence of parsecmgmt program runs and save the
    Dataframes of data within files.
    ...
    data = ParsecData()
    data.config = {'pkg': args.package, 'execdate': execdate,
